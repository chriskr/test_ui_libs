...

{
   ...
   * with 0.
   *
   * E.g. Date.getWeekNumbers(2017, 10) returns
   *
   *   [
   *     [44, [0, 0, 1, 2, 3, 4, 5]],
   *     [45, [6, 7, 8, 9, 10, 11, 12]],
   ...
   *
   */
  Date.getWeeksOfMonth = function(year, month, startWithMonday = true) {
    let day = 1;
    const date = new this(year, month, day);
    ...
    while (day <= dayCount) {
      ...
      week.push(day++);
      if (week.length === 7) {
        weeks.push([date.getWeekNumber(), week]);
        week = [];
      }
    }
